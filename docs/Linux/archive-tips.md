...
!!!note
    флаг `-С` не имеет отношения к типу сжатия, а указывает на переход в определенный каталог перед сжатием. Мне надо сжать каталог `/sites/youtrack`, но чтобы внутри архива был не полный путь, а только каталог youtrack. Я заранее перехожу в каталоге `/sites` и указываю, что архивировать буду папку youtrack.
