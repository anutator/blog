...
По документации: IP address: 192.168.1.2, маска 255.255.255.0, шлюз 192.168.1.1
По факту: статического адреса не было, по умолчанию DHCP. Надо на компьютере запускать DHCP сервер TFTPd32.
Логин/пароль=**Admin/2n**. Пока не меняла то что по умолчанию.
...
После входа задать статический адрес: 10.8.13.134 маска 255.255.255.224шлюз 10.8.13.129. DNS1 10.8.177.28, DNS2 10.8.175.5. Сохранить.
...
![](gsm-license-default.png)  

После покупки, у 2N запрашивается лицензия для разблокировки ограничения по времени. Пример лицензионного файла **licence_M201-5415860438.key**:
...
SIP proxy (IP→GSM) предназначен для безопасного соединения с трафиком только между Avaya CM. Можно задать ip адрес и порт, с которого будут приниматься SIP пакеты. Если оставить `0.0.0.0`, он будет открыт для любого трафика.
...
Предположим, что у оператора GSM в России префикс 8 и с общим числом цифр — 11.
...
Во входящих  GSM группах можно определить поведение для каждой входящей группы GSM. Выбрать режим Reject, Ignore, Accept incoming calls или Callback. Выбираем **Accept incoming calls + dialtone**.

Можно определить список автоматически обзваниваемых номеров Avaya при отсутствии тонального набора по истечении DTMF dialing timeout, если человек ничего не нажал. Нам список не нужен, а только один номер.

В настройках таймаут ставим 0 секунд (DTMF dialing timeout), чтобы звонок сразу шел на внутренний номер 7799 (проверить, что настроен SIP proxy (GSM→IP) в параметрах VoIP). 

GSM incoming groups - выбрана группа 1:
...
Пролистываем ниже. List of called numbers: 7799 — номер VDN в Avaya, который приходит на вектор 1 (приветствие с донабором внутреннего номера).
...
**ПРОБЛЕМА ТОНАЛЬНОГО НАБОРА**: для корректного тонального донабора внутренних номеров надо увеличить время ожидания в первом шаге вектора с2 до 3 секунд. Когда было 2 секунды, при донабонре первая цифра часто задваивалась, например набираю 7095 или 7098, а в векторе 7709, и звонок не проходит куда надо. Помогло, когда донабираем внутренний номер сразу после начала приветствия.
...
На текущий момент выделила только префикс 8916, чтоб не касалось остальных звонков. Если нужно тестировать с другими, можно добавить в другой route pattern или создать свои префиксы.

``` title="li ars an loc 4"
...
![https://faq.2n.cz/download/attachments/7353563/asterisk_1.PNG?version=1&modificationDate=1362131500973&api=v2](assets/image163.png)

### SIP транк 2N VoiceBlue Next
Для настройки транка между **2N VoiceBlue Next** и Asterisk, настроить `SIP proxy (GSM→IP`) для входящих звонков GSM. `SIP proxy(IP→GSM)` предназначен только для безопасного соединения с трафиком отAsterisk. Можно задать IP адрес и порт, на котором будут приниматься IP-пакеты.

Если здесь оставить `0.0.0.0`, трафик будет небезопасным.
...
![https://faq.2n.cz/download/attachments/7353563/asterisk_2.PNG?version=2&modificationDate=1397564885503&api=v2](assets/image164.png)

![https://faq.2n.cz/download/attachments/7353563/asterisk_3.PNG?version=1&modificationDate=1362132248583&api=v2](assets/image165.png)
...
![https://faq.2n.cz/download/attachments/7353563/asterisk_4.PNG?version=1&modificationDate=1362132285330&api=v2](assets/image166.png)
...
![https://faq.2n.cz/download/attachments/7353563/asterisk_5.PNG?version=1&modificationDate=1362132333573&api=v2](assets/image167.png)
...
![https://faq.2n.cz/download/attachments/7353563/asterisk_7.PNG?version=1&modificationDate=1362132509380&api=v2](assets/image169.png)
...
![https://faq.2n.cz/download/attachments/7353563/asterisk_8.PNG?version=1&modificationDate=1362132591820&api=v2](assets/image170.png)
...
![https://faq.2n.cz/download/attachments/7353563/asterisk_9.PNG?version=1&modificationDate=1362132624883&api=v2](assets/image171.png)
...
![https://faq.2n.cz/download/attachments/7353563/asterisk_10.PNG?version=1&modificationDate=1362132865103&api=v2](assets/image172.png)
...
Для применение ввести `sip reload`.
...
После теста немного подождать перед скачиванием файла. Просмотреть в онлайне внутреннюю трассировку: *Gateway→ Gateway control→ Online report* (не отобразилось)
...
- Non-standardised SMS DLR support - incoming SMS as DLR
- Non-standardised SMS DLR support - changed prefix match
...
For further product information see please the latest user guides andFAQs at http://wiki.2n.cz
...
For further details about the new routing configuration see please thelatest user guides and FAQs at http://wiki.2n.cz

Other new features:
- Heavy SMS messaging optimalisation for multiple SMPP accounts
...
New features:
- Blocked SIM card detection/Failed calls - based on selecteddisconnection/release causes only (with 2NSIM Star Server)
- Repeated GSM network registration tries (1-50 tries) withregistration timeout (0-3600s)
- Audio gain settings from the web interface (2NStarGate/BlueToweronly)
...
![https://2nwiki.2n.cz/download/attachments/7353385/VBN_03.jpg?version=1&modificationDate=1447669257117&api=v2](assets/image199.jpeg)

## Тест
**Тест — набираю 8916**0239103
